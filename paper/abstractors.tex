...
\end{aug}
% OLD ABSTRACT FROM ARXIV SUBMISSION
% \begin{abstract}
% A framework is proposed that casts relational learning in terms of transformers,
% implementing binding between sensory states and abstract states
% with relational cross attention mechanisms.
% \end{abstract}

% CURRENT DRAFT OF ABSTRACT
\begin{abstract}
    In recent years, a stream of research efforts has appeared in machine learning focusing on \textit{relational models}. Relational models are sequence models which are designed to learn and model \textit{relations} between objects more explicitly, enabling more sample-efficient learning for ``relational tasks''. Transformers are very powerful sequence models characterized by `attention' mechanisms. In this paper we propose the \textit{Abstractor} framework and module, which augments relational processing in transformer-based models. We theoretically analyze the class of relation functions Abstractors can compute and empirically demonstrate superior sample-efficiency on relational tasks compared to standard Transformer models.
\end{abstract}
\end{frontmatter}
