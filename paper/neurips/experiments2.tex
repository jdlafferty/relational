...
% JDC: I'M A BIT UNCOMFORTABLE WITH THE STATEMENT AS IT WAS, GIVEN WHAT WE KNOW ABOUT THE NORMALIZATION ISSUE.
% SUGGEST THE ADDITION BELOW AND, IF AT ALL POSSIBLE (GIVEN SPACE) THE ENSUING FOOTNOTE.
One is that
, as described,
it is only able to model symmetric relations---$A$ is symmetric by definition.
% JDC: IF POSSIBLE INSERT THE FOLLOWING FOOTNOTE JUST AFTER "symmetric relations" ABOVE:
% \footnote{This limitation arises from row-wise normalization when evaluating the similarity matrix; eliminating that
% can allow CorelNet to model asymmetric relations, however that form of the model has not yet been fully tested with
% respect to its other capabilities.
