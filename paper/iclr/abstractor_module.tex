...
\begin{equation}\label{eq:relational_crossattn}
    X' \gets  \sigma_{\mathrm{rel}}\paren{\phi(X) \, {\psi(X)}^\top} S,
\end{equation}
...
Multi-dimensional relations can be modeled through multi-head relational cross-attention. In our experiments, $\phi_i, \psi_i$ are linear maps $W_1^{i}, W_2^{i}$, and multi-head relational cross-attention is given by
\begin{equation}
    \begin{split}
        ...
        \text{where } A^i &= \sigma_{\mathrm{rel}}\paren{(X \, W_1^{i}) {(X \, W_2^{i})}^\top} S \, W_o^{i} .
    \end{split}
\end{equation}
