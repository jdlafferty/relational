...
We propose three mechanisms of assigning symbols to objects. The key property that symbol assignment mechanisms must satisfy is to \textit{refer to} the object, without encoding its features.

\textbf{Positional symbols.} The simplest symbol assignment mechanism which satisfies this property is to simply assign each object a symbol based on the position it appears in the sequence. That is, we maintain a library of symbols $S_{\mathrm{lib}} = (s_1, \ldots, s_{\mathtt{max\_len}}) \in \reals^{d_s \times \mathtt{max\_len}}$, and assign the symbol $s_i$ to the $i$-th object. $S_\mathrm{lib}$ can be either learned parameters of the model or nonparametric positional embeddings (e.g., sinusoidal). While very simple, positional symbols are empirically powerful.
...
% We learn a library of symbols $S_{\mathrm{lib}} = (s_1, \ldots, s_{n_s}) \in \reals^{d_s \times n_s}$, where $n_s$ is the number of symbols in the library. For each symbol $s_i$, we learn a corresponding binding vector $b_i$, forming a library of binding vectors $B_{\mathrm{lib}} = (b_1, \ldots, b_{n_s})$. Through an attention mechanism, symbols are bound to objects $x_i$ based on relations to binding vectors. A multi-head variant of symbolic attention is defined naturally by concatenating the symbols retrieved for each head. That is, 
% \begin{equation}
%     \begin{split}
%         S &\gets \mathrm{concat}\paren{S^{(1)}, \ldots, S^{(n_h)}}, \quad \text{where,}\\
%         S^{(i)} &\gets S_{\mathrm{lib}}^{(i)} \mathrm{Softmax} \paren{(W_q^{(i)} X)^\top B_{\mathrm{lib}}^{(i)}}.
%     \end{split}
% \end{equation}
% In terms of representational capacity, this is similar to cross-attending to $S_{\mathrm{lib}}$ (essentially, $B_{\mathrm{lib}}$ is replaced by $W_k S_{\mathrm{lib}}$). Note that this weakens the relational bottleneck since object-level features are used to retrieve a symbol for each object. However, the dependence is only with respect to a low-dimensional projection of the object-level features, which we think of as encoding the object's ``syntactic role.'' Encoding such information in the symbols allows identifying objects by their role.

Ultimately, this results in the Abstractor state $A_i$ encoding the relations between object $i$ and each object $j$, identifying $j$ by its position, relative-position, or ``syntactic role,'' depending on the symbol-assignment mechanism used.
...
\begin{algorithm}[ht!]
    ...
    $A^{(0)} \gets \mathrm{CrossAttention(X, S_{\mathrm{lib}})}$ \quad \texttt{// or, $A^{0} \gets S_{1:m}$, for positional symbols}
