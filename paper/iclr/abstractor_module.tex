...
\end{equation}
Symbols act as abstract references to objects. They do not contain any information about the contents or features of the objects, but rather \textit{identify} objects via their position. The symbols $S = (s_1, s_2, \ldots)$ can be either learned parameters of the model or nonparametric positional embeddings. Those vectors are `symbols' in the same sense that we call $x$ a symbol in an equation like $y = x^2$---they are a reference to an object with an unspecified value. The difference is that the symbols here are `distributed representations' (i.e., vectors), so this gives a novel perspective on the long-standing problem of neural versus symbolic computation.
