...
The core operation in a Transformer is attention. For an input sequence $X = (x_1, \ldots, x_\m)$, self-attention transforms the sequence via, $ X' \gets \phi_v(X) \, \mathrm{Softmax}({\phi_q(X)}^\top \phi_k(X))$,
% \begin{equation}\label{eq:self_attn}
%     X' \gets \phi_v(X) \, \mathrm{Softmax}\paren{\phi_q(X)^\top \phi_k(X)},
% \end{equation}
where $\phi_q, \phi_k, \phi_v$ are functions on $\calX$ applied independently to each object in the sequence.
