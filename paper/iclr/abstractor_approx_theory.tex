...
\begin{remark}
    In~\Cref{eq:relational_crossattn}, we formulate relational cross-attention with the maps $\phi_q, \phi_k$ as linear projections, whereas $\phi_q, \phi_k$ are multi-layer perceptrons in~\Cref{eq:abstractor_layer}. However, recall that the input to each Abstractor layer is the output of the preceding layer which ends with a multi-layer perceptron. The function class of a multi-layer perceptron followed by two different linear projections is the same as the function class of two different multi-layer perceptrons. We focus on~\Cref{eq:abstractor_layer} for ease of presentation.
\end{remark}
