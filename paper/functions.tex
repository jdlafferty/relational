...
\begin{thm}[Function class of inner product relational neural networks]
	...
	\begin{equation*}
		\langle x, y \rangle_{\NN} := \begin{bmatrix}\langle \NN_{\theta_1}(x), \NN_{\theta_1}(y) \rangle \\ \vdots \\ \langle \NN_{\theta_{d_r}}(x), \NN_{\theta_{d_r}}(y) \rangle\end{bmatrix}.
	\end{equation*}
...
\begin{proof}
	...
	\begin{equation}
		...
		\sup_{x\in \mathcal{X}}{\left\lvert (\NN(x))_j - \sqrt{\lambda_j} \psi_j(x) \right\lvert } < \epsilon_1
	\end{equation}
	...
	For ease of notation, we denote \(\NN_{\hat{\theta}_i}\) simply by \(\NN\), omitting the dependence on fixed \(i\). Furthermore, \(\NN(x)_j\) is the \(j\)th component of the output of \(\NN(x)\). Now note that the approximation error for \(R_i\) is bounded by
	\begin{equation}
		\label{eq:proof_approx_bound}
		\begin{split}
			\sup_{x, y \in \mathcal{X}}&{
				\left\lvert R_i(x,y) - \langle \NN(x), \NN(y) \rangle \right\rvert}\\
			&= \sup_{x, y \in \mathcal{X}}{
				\left\lvert R_i(x,y) - \sum_{j=1}^{\tilde{n}_i}{\NN(x)_j \NN(y)_j} \right\rvert} \\
			&\leq \sup_{x,y \in \mathcal{X}}{ \left(
				\left\lvert R_i(x,y) - \sum_{j=1}^{\tilde{n}_i}{\lambda_j \psi_j(x) \psi_j(y)} \right\rvert
				+ \left\lvert \sum_{j=1}^{\tilde{n}_i}{\lambda_i \psi_j(x) \psi_i(y) - \NN(x)_j \NN(y)_j} \right\rvert  \right) }
		\end{split}
	\end{equation}
	...
	\begin{equation*}
		\begin{split}
			&\left\lvert \left(\sum_{j=1}^{\tilde{n}_i}{\lambda_i \psi_j(x) \psi_i(y)}\right) - \langle \NN(x), \NN(y) \rangle \right\rvert  \\
			&\leq \sum_{j=1}^{\tilde{n}_i}{ \left\lvert \lambda_i \psi_j(x) \psi_i(y) - \NN(x)_j \NN(y)_j \right\rvert} \\
			&\leq \sum_{j=1}^{\tilde{n}_i}{\left(
				\left\lvert \NN(x)_j \right\rvert \left\lvert \sqrt{\lambda_j} \psi_j(y) - \NN(y)_j \right\rvert
				+ \lvert \NN(y)_j \rvert \left\lvert \sqrt{\lambda_j} \psi_j(x) - \NN(x)_j \right\rvert
				\right)}
		\end{split}
	\end{equation*}
	...
	\begin{equation*}
		\sup_{x, y \in \mathcal{X}}{
			\lvert R_i(x,y) - \langle \NN(x), \NN(y) \rangle \rvert} \leq \frac{\epsilon}{2} + \frac{\epsilon}{2} = \epsilon
	\end{equation*}

	We repeat this for each component of the relation function \(R_i\), \(i=1, \ldots, d_r\), obtaining \(d_r\) neural networks each with parameters \(\hat{\theta}_i\). Thus, \(\sup_{x,y \in \mathcal{X}}{\lVert R(x,y) - \langle x, y \rangle_\NN \rVert_\infty} < \epsilon\).
\end{proof}
